...
class ULR(nn.Module):
    ...
    def __init__(self,params):
        super().__init__()
        self.words_num = params.ulr_words_num
        self.emb_dim = params.emb_dim
        #self.pad_index = params.pad_index
        self.embeddings = Embedding(self.words_num, self.emb_dim)
        self.A = nn.Linear(self.emb_dim,self.emb_dim,bias=False)
        nn.init.eye_(self.A.weight)
        self.temp = params.temperature
        self.softmax = nn.Softmax(dim=1)
    def forward(self,x,pad_mask,topk=10):
        bs,slen,dim = x.shape
        #ulr_x = x.clone().fill_(0).reshape(-1,x.shape[2]) #(bs*seq,emb_dim)
        x_reshape = x.reshape(bs*slen,dim)
        ulr_x = x_reshape.clone().fill_(0)
        pad_mask_flat = pad_mask.flatten() #bs*seq
        ...
        q = D
        #topk
        value,_ = q.topk(k=topk,dim=1)
        value_min = torch.min(value,dim=1)[0].reshape(-1,1)
        
        topk_mask =  ( q >= value_min )
        
        
        
        masked_q = q.clone().fill_(-float('inf'))
        masked_q[topk_mask] = q[topk_mask]
        masked_q = self.softmax(masked_q)
        
        Eu = torch.matmul(masked_q,self.embeddings.weight) #(query_num,emb_dim)
        ulr_x[pad_mask_flat] = Eu

        return ulr_x.reshape(x.shape)
